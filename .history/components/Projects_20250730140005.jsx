...
const ProjectShowcase = () => {
  ...
  return (
    <>
      ...
      <section className="relative pt-4 md:pt-32 min-h-screen overflow-hidden bg-black">
        ...
        {/* Reverted desktop positioning, added mobile-specific positioning */}
        <div className="absolute top-15 right-0 md:flex justify-center w-full mb-4 md:mb-0 z-10 hidden md:block">
           <Image
        ...
        {/* Mobile-only image positioning (displays only on small screens) */}
        <div className="relative flex justify-center w-full mb-4 z-10 md:hidden">
          <Image
            src="/assets/fishing.png"
            alt="Decorative fishing illustration"
            width={162.5}
            height={222.3}
            className="w-12 h-auto" // Even smaller on mobile (w-12)
          />
        </div>
        ...
        {/* Hero-style Heading */}
        {/* Added dynamic margin-top to adjust for image on mobile */}
        <div className="relative z-10 px-4 md:px-16 mt-0 md:mt-20">
          <h1 className="text-4xl md:text-8xl font-bold leading-tight text-center mb-6"> {/* Slightly smaller heading on mobile */}
            <span className="text-orange-500">PROJECT </span>
            <span className="text-white">SHOWCASE</span>
          </h1>

          {/* Category Filters (Mobile: Dropdown) */}
          {/* Increased mt for dropdown on mobile, added z-index to dropdown div */}
          <div className="flex justify-center mt-4 gap-6 flex-wrap md:hidden relative z-10">
            ...
            {isDropdownOpen && (
              <div className="absolute top-full mt-2 w-full max-w-[200px] bg-gray-900 rounded-md shadow-lg z-50 border border-gray-700 animate-fadeInUp mx-auto overflow-hidden"> {/* Changed z-index to z-50 for dropdown */}
                {categories.map((cat) => (
                  <button
                    key={cat}
        ...
        {/* 3D Carousel */}
        {/* Adjusted mt for carousel on mobile to ensure dropdown space */}
        {/* min-h increased to provide ample space for dropdown even when it expands */}
        <div className="relative w-full mt-8 md:mt-10 px-4 z-10">
          <div
