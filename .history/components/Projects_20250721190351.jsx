...
const ProjectCard = ({ project, className }) => {
  return (
    <div className={`bg-[#1f2937] rounded-2xl p-6 max-w-xl w-full text-white shadow-lg ${className}`}>
      <img
        src={project.image}
        alt={project.title}
...
const ProjectShowcase = () => {
  ...
  return (
    <section className="relative pt-40 md:pt-52 min-h-screen bg-black">
      ...
      <div className="flex justify-center mt-12 gap-4 flex-wrap">
        {categories.map((cat) => (
          <button
            key={cat}
            className={`rounded-full px-6 py-2 text-lg transition-all duration-300 shadow-md ${
      ...
      {/* Carousel */}
      <div className="relative w-full max-w-6xl mx-auto mt-20 h-[540px] overflow-hidden">
        ...
        {/* Nav Arrows */}
        <button
          className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-[#111827] p-3 rounded-full shadow-lg z-10 hover:scale-110 transition"
          onClick={handlePrev}
        >
          <ChevronLeft className="text-white" />
        </button>
        <button
          className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-[#111827] p-3 rounded-full shadow-lg z-10 hover:scale-110 transition"
          onClick={handleNext}
        >
