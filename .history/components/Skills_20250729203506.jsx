...
// Updated skillsData to match resume categories
const skillsData = {
...
// Updated skillsDetails to match resume content
const skillsDetails = {
  LANGUAGES: {
...
const Skills = () => {
  ...
  // State for skill card visibility and content
  const [showSkillCard, setShowSkillCard] = useState(false);
  const [currentSkillCategory, setCurrentSkillCategory] = useState(null);

  useEffect(() => {
    // Only define @keyframes fall and fade-in in the stylesheet.
    // The individual animation properties will be applied via the style prop.
    ...
    styleElement.textContent = `
      ...
      /* Removed .star-fall animation: fall linear infinite; from here */
    `;
  ...
  const handlePlanetClick = (category) => {
    ...
    setShowSkillCard(true); // This makes the card visible
  };

  // Close the skill card
  const handleCloseSkillCard = () => {
    setShowSkillCard(false); // This hides the card
    setCurrentSkillCategory(null); // Clear selected category
  };
  ...
  return (
    <>
      ...
      >
        {/* Animated Stars Background - Lower its z-index */}
        <div className="absolute inset-0 overflow-hidden z-0"> {/* Changed z-20 to z-0 */}
          {stars.map((star) => (
            <div
              key={star.id}
              className="absolute rounded-full bg-white" // Removed star-fall class here
              style={{
                left: `${star.x}%`,
                top: `${star.y}%`,
                width: `${star.size}px`,
                ...
                // Apply all animation properties directly in style prop
                animationName: 'fall',
        ...
        {/* 🪐 Heading - Ensure it's above stars but below planets */}
        ...
        {/* 🌠 Orbit System - Ensure this is above the stars background */}
        <div className="relative h-[500px] w-[100vw] max-w-none overflow-hidden mx-auto flex items-center justify-center z-20"> {/* Changed z-10 to z-20 */}
          {/* 🌟 Central Star - Keep it within its parent's stacking context or adjust if needed */}
          <img
            src="assets/star.png"
            alt="Central Star"
            className="absolute w-24 sm:w-32 md:w-40 h-auto z-30 object-contain" // Gave it a higher z-index to be on top of planets if they overlap visually
          />
