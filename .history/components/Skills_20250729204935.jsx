...
const Skills = () => {
  ...
  return (
    <>
      ...
      {/* Skill Details Card - Added Background Image with Low Opacity */}
      {showSkillCard && detailsToShow && (
        ...
        >
          ...
          {/* Actual Skill Card - Background image and opacity added here */}
          <div
            className="relative text-white p-6 sm:p-8 rounded-xl shadow-2xl max-w-2xl w-full mx-auto
                       ...
                       bg-cover bg-center bg-no-repeat" // Added background image properties
            onClick={(e) => e.stopPropagation()}
            style={{
              backgroundImage: `url('/assets/skillCard.jpg')`, // Set the background image
              backgroundBlendMode: 'overlay', // Blend mode for better text readability
              backgroundColor: 'rgba(0,0,0,0.6)', // Fallback/base background color for transparency
              // Use backdrop-filter directly if you want only the image to be low opacity and content to be 100%
              // or apply a class for general opacity if the whole card needs low opacity.
              // For a subtle background image that doesn't affect text opacity, we need to create an inner div.
              // Let's create an inner div for the background to control its opacity separately.
            }}
          >
            {/* Inner div for the background image with low opacity */}
            <div
              className="absolute inset-0 rounded-xl"
              style={{
                backgroundImage: `url('/assets/skillCard.png')`,
                backgroundSize: 'cover',
                backgroundPosition: 'center',
                backgroundRepeat: 'no-repeat',
                opacity: 0.15, // Low opacity for the background image
            ...
            {/* Close Button */}
            <button
              className="absolute top-4 right-4 sm:top-5 sm:right-5 text-gray-400 hover:text-white text-4xl sm:text-5xl transition-colors duration-200"
              onClick={handleCloseSkillCard}
