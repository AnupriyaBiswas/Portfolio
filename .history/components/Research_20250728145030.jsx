...
// Define target rotation angles for the spotlight for each card.
// These will likely need fine-tuning based on your spotlight image and layout.
// Adjusted for potentially smaller cards and overall vertical fit.
const spotlightAngles = [-15, 0, 15]; // Angle for Card 1 (top), Card 2 (middle), Card 3 (bottom)
// You might need to adjust these more: e.g., [-20, 0, 20] or [-10, 5, 20]
// based on the exact pivot point and size of your spotlight.jpg image.
...
const Research = () => {
  ...
  // Get the current spotlight angle
  const currentRotationAngle = spotlightAngles[currentSpotlightIndex];

  return (
    <>
      <section
        id="research"
        className="min-h-screen bg-black text-white relative overflow-hidden w-screen flex flex-col justify-center items-center py-10 sm:py-20" // Reduced top/bottom padding
      >
        ...
        <div className="pt-10 pb-8 text-center z-10 relative px-4 w-full"> {/* Reduced padding */}
          <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold tracking-tight"> {/* Reduced font size */}
            <span className="text-orange-500">ASTRO-CODEX</span>{" "}
            <span className="text-white">INTERFACE</span>
          </h2>
          <p className="text-gray-400 text-base md:text-lg mt-2 font-light max-w-2xl mx-auto"> {/* Reduced font size and margin */}
        ...
        <div className="relative z-10 w-full flex items-center justify-center min-h-[60vh] md:min-h-[70vh] lg:min-h-[80vh] py-4"> {/* Adjusted min-h and added py */}
            {/* Left Side: Spotlight Image */}
            {/* The spotlight image needs to be positioned relative to its container to allow for rotation and to simulate shining across. */}
            {/* Ensure 'spotlight.png' is placed in your 'public/assets/' folder. */}
            <div className="hidden md:flex w-[40%] h-full items-center justify-end relative pr-8">
                <img
                    src="/assets/spotlight.png" // CONFIRM THIS PATH: e.g., /spotlight.png if in public root, or /assets/spotlight.png
                    alt="Spotlight"
                    // Adjust max-h based on the actual height of your spotlight image
                    className="absolute right-0 top-1/2 -translate-y-1/2 h-auto max-h-[80%] object-contain transition-transform duration-700 ease-in-out origin-[100%_50%]" // Adjusted origin for pivot from right edge
                    style={{ transform: `rotate(${currentRotationAngle}deg)` }}
                />
            </div>

            {/* Right Side: Research Data Streams */}
            {/* Adjusted max-h to ensure cards fit, and added justify-evenly */}
            <div className="w-full md:w-[60%] px-4 md:px-0 flex flex-col justify-evenly h-full max-h-[calc(100vh-200px)] overflow-hidden"> {/* max-h adjusted for header/footer */}
                {researchItems.map((item, index) => (
                    ...
                    >
                        ...
                        <div className="absolute top-1/2 left-3 md:left-4 transform -translate-y-1/2 text-orange-400 text-2xl md:text-3xl font-mono opacity-60
                        ...
                        {/* Content */}
                        <div className="relative z-10 pl-12 md:pl-16"> {/* Adjusted padding for smaller index */}
                            <h3 className="text-lg md:text-xl font-bold mb-1 text-white group-hover:text-orange-100 transition-colors duration-300 leading-tight"> {/* Smaller title font */}
                                {item.title}
                            </h3>

                            <p className="text-gray-300 mb-2 text-sm leading-relaxed line-clamp-2"> {/* Smaller summary font, clamp to 2 lines */}
                                {item.summary}
                            </p>
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-gray-800/50">
                                ...
                                {/* Status badge */}
                                <div className="flex items-center space-x-1 text-xs"> {/* Smaller status font */}
                                    <div
        ...
        <div className="mt-8 flex justify-center relative z-10"> {/* Reduced margin-top */}
        ...
        <p className="text-gray-500 text-xs mt-2 relative z-10">End of Transmissions</p> {/* Reduced font size and margin */}
      </section>
    </>
  );
};
