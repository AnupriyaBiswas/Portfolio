...
import { ChevronLeft, ChevronRight, Code, Brain, HardDrive } from "lucide-react";
import Image from "next/image"; // Correct import for optimized images
...
const ProjectCard = ({ project, activeIndex, index, totalProjects, onClick }) => {
  ...
  const getTransform = () => {
    ...
    if (absDistance === 1) {
      translateXValue =
        distance > 0
          ? `${desktopTranslateX1}px`
          : `${-desktopTranslateX1}px`;
    } else if (absDistance === 2) {
      translateXValue =
        distance > 0
          ? `${desktopTranslateX2}px`
          : `${-desktopTranslateX2}px`;
    } else {
