    ...
    <>
      ...
      >
        ...
        <style jsx>{`
          ...
          /* Modified animations for unified side alignment */
          @keyframes fadeIn {
            from {
              opacity: 0;
              transform: translateX(0); /* Start directly above */
            }
            to {
              opacity: 1;
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto py-12 px-4">
          ...
          {educationItems.map((item, index) => (
            <div
              key={item.id}
              // Removed alternating justify-start/justify-end
              className={`relative mb-10 flex items-center w-full justify-center`} // All items centered around the line
            >
              <div
                // All cards are now md:w-6/12 (50%) and positioned relative to the center line
                // Removed background, shadow, and border as requested earlier
                className={`w-full md:w-6/12 p-4 rounded-lg
                            transform transition-all duration-700 ease-in-out
                            ${
                              index % 2 === 0
                                ? "md:pr-8 md:text-right" // Right aligned text for left card
                                : "md:pl-8 md:text-left" // Left aligned text for right card
                            }
                            animate-[fadeIn_1s_ease-out_forwards]`} // Unified fadeIn animation
