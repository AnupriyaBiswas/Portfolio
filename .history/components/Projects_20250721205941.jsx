...
const ProjectCard = ({ project, activeIndex, index, totalProjects, onClick }) => {
  ...
  return (
    <div
      className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-700 ease-out" // Centered vertically and horizontally
      style={{
        transform: `translateX(-50%) translateY(-50%) ${getTransform()}`, // Included translateY for centering
        zIndex: getZIndex(),
        opacity: getOpacity(),
...
const ProjectShowcase = () => {
  ...
  const handleTouchStart = (e) => {
    setTouchStartX(e.targetTouches[0].clientX); // Corrected access to clientX
  ...
  const handleTouchEnd = (e) => {
    setTouchEndX(e.changedTouches[0].clientX); // Corrected access to clientX
    ...
    <>
      <style jsx>{`
        ...
        @media (max-width: 767px) {
          ...
          /* These styles are applied directly to the ProjectCard's root div via data-distance */
          /* The transform property is already handled by the JS getTransform() function,
             which now explicitly includes translateY(-50%) for vertical centering.
             The !important is removed as it's no longer necessary with the updated JS logic
             and direct style application.
          */
        }
      `}</style>
      ...
      <section className="relative pt-20 md:pt-32 min-h-screen overflow-hidden bg-black">
        {/* Top Right Image */}
        <div className="absolute top-15 right-4 z-50"> {/* Moved down from top-4 to top-8 */}
          <Image src="/assets/fishing.png" alt="Decorative fishing illustration" width={162.5} height={222.3} /> {/* Adjusted size to maintain aspect ratio */}
        </div>
        ...
        <div className="relative z-10 px-4 md:px-16">
          <h1 className="text-6xl md:text-8xl font-bold leading-tight text-center mb-6"> {/* Reduced mb-8 to mb-6 */}
            <span className="text-orange-500">PROJECT </span>
            <span className="text-white">SHOWCASE</span>
          </h1>

          {/* Category Filters (Mobile: Dropdown) */}
          <div className="flex justify-center mt-4 gap-6 flex-wrap md:hidden relative"> {/* Reduced mt-8 to mt-4 */}
            <button
          ...
          <div className="hidden md:flex justify-center mt-4 gap-10 flex-wrap"> {/* Reduced mt-8 to mt-4 */}
            {" "}
        ...
        <div className="relative w-full mt-6 md:mt-10 px-4"> {/* Reduced mt-10 md:mt-20 to mt-6 md:mt-10 */}
          <div
            className="relative h-[350px] md:h-[500px] max-w-full md:max-w-7xl mx-auto flex items-center justify-between" // Adjusted height and kept flex alignment
            onMouseEnter={() => setIsAutoPlaying(false)}
            onMouseLeave={() => setIsAutoPlaying(true)}
            onTouchStart={handleTouchStart} // Add touch event listeners
            onTouchMove={handleTouchMove}
            onTouchEnd={handleTouchEnd}
            style={{ perspective: "1200px", minHeight: '350px' }} // Ensured minHeight for when there are no projects
          >
            {/* Left Arrow */}
            <button
            ...
            <div className="relative flex justify-center items-center h-full w-full"> {/* Ensured full height and width for centering cards */}
