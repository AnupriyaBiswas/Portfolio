...
// Updated skillsData (no change needed here for card design)
const skillsData = {
...
// Updated skillsDetails (no change needed here for card design)
const skillsDetails = {
  LANGUAGES: {
...
const Skills = () => {
  ...
  useEffect(() => {
    // Add CSS animations to document head (no change here from last fix)
  ...
  return (
    <>
      ...
      {/* Skill Details Card - DESIGN REFINEMENT HERE */}
      {showSkillCard && detailsToShow && (
        ...
        >
          ...
          {/* Actual Skill Card - Updated Styling */}
          <div
            className="relative bg-slate-900 text-white p-6 sm:p-8 rounded-xl shadow-2xl max-w-lg w-full mx-auto
                       border border-orange-600/50 shadow-orange-500/30 animate-fade-in
                       transform transition-all duration-300 ease-out scale-100" // Added transition-all for potential future hover/active states
          ...
          >
            ...
            {/* Skill List */}
            <ul className="list-none space-y-3 text-lg sm:text-xl px-4"> {/* Removed default list-disc for custom bullet */}
              {detailsToShow.details.map((skill, index) => (
                <li key={index} className="flex items-center">
                  <span className="mr-3 text-orange-300 text-2xl">⚡</span> {/* Custom bullet point */}
                  {skill}
                </li>
              ))}
            </ul>
            ...
            {/* Close Button */}
            <button
              className="absolute top-3 right-3 sm:top-4 sm:right-4 text-gray-400 hover:text-white text-4xl sm:text-5xl transition-colors duration-200"
              onClick={handleCloseSkillCard}
