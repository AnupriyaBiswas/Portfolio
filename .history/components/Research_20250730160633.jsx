...
const Research = () => {
  ...
  const startCardHighlightOscillation = useCallback(() => {
    ...
    spotlightIntervalRef.current = setInterval(() => {
      if (!isHovering) {
        setCurrentSpotlightIndex((prevIndex) => (prevIndex + 1) % researchItems.length);
      }
    }, 4000);
  }, [isHovering]);
          ...
          <div className="w-full md:w-[70%] px-4 flex flex-col justify-between h-full max-h-[calc(100vh-250px)] md:max-h-[calc(100vh-200px)] lg:max-h-[calc(100vh-150px)]">
            {researchItems.map((item, index) => (
              ...
              >
                ...
                {/* Glow Border */}
                <div
                  className="absolute inset-0 border-2 border-transparent rounded-lg pointer-events-none md:group-hover:border-orange-500/80 transition-all duration-500"
                  style={
