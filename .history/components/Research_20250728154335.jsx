      ...
      >
        ...
        <style jsx>{`
          ...
          @keyframes subtleGlowPulse {
            0%,
            100% {
              box-shadow: 0 0 15px rgba(249, 115, 22, 0.6); /* Increased glow intensity */
            }
            50% {
              box-shadow: 0 0 30px rgba(249, 115, 22, 0.9); /* Increased glow intensity */
            }
          }
        ...
        <div className="relative z-10 w-full flex items-center justify-center min-h-[calc(100vh-80px)] md:min-h-[calc(100vh-80px)] py-4"> {/* Corrected min-h syntax */}
          ...
          <div className="hidden md:flex w-[40%] h-full items-center justify-center relative overflow-hidden">
            <img
              src="/assets/spotlight.jpg" // Changed from .png to .jpg as per previous feedback. Verify exact path if needed.
            ...
            {researchItems.map((item, index) => (
              <div
                ...
                className={`group relative my-2 p-4 md:p-5 rounded-lg overflow-hidden
                                     bg-gradient-to-r from-gray-800/80 via-gray-800/70 to-transparent /* Made background slightly less dark */
                                     border border-gray-700/60 /* Brighter default border */
                                     transition-all duration-500 cursor-pointer
                                     shadow-lg hover:shadow-xl hover:shadow-orange-500/25 /* Stronger hover shadow */

                                     // Default (mobile) state: no hover effects
                                     opacity-100 scale-100
                                     
                                     // md and larger: apply initial faded state and hover effects
                                     md:opacity-60 md:scale-95 /* Increased base opacity from 40 to 60 */
              ...
              >
                ...
                <div className="absolute inset-0 border-2 border-transparent rounded-lg pointer-events-none
                                     md:group-hover:border-orange-500/70 transition-all duration-500" /* Brighter hover border */
                  style={currentSpotlightIndex === index ? { animation: `subtleGlowPulse 2s infinite ease-in-out` } : {}}>
                </div>

                {/* Data Index / Chrono-Log Number */}
                <div className="absolute top-4 left-4 text-orange-300 text-xl font-bold opacity-70"> {/* Brighter orange, higher opacity */}
                  {index + 1}
                </div>
                ...
                <div className="relative z-10 pl-12 md:pl-16">
                  ...
                  <p className="text-gray-200 mb-2 text-sm leading-relaxed line-clamp-2"> {/* Changed to gray-200 for brighter text */}
                    {item.summary}
                  </p>

                  <div className="flex items-center justify-between pt-2 border-t border-gray-700/50"> {/* Slightly brighter border */}
                    <a
                      href={item.link}
                      target="_blank"
                    ...
                    <div className="flex items-center space-x-1 text-xs">
                      <div
                        className={`w-2 h-2 rounded-full ${item.status === "Published"
                            ? "bg-green-400" /* Slightly brighter green */
                            : "bg-orange-400" /* Slightly brighter orange */
                          } animate-pulse-fast`}
                      ></div>
                      <span
                        className={`${item.status === "Published"
                            ? "text-green-300" /* Slightly brighter green text */
                            : "text-orange-300" /* Slightly brighter orange text */
                          } font-medium`}
                      >
                        {item.status}
                      </span>
