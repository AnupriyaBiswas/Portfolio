      ...
      {visibleConstellations.map((c, i) => (
        ...
        >
          <svg 
            ...
            onMouseEnter={() => !isMobile && setHoveredConstellation(c.name)}
            onMouseLeave={() => !isMobile && setHoveredConstellation(null)}
          >
          ...
          {!isMobile && hoveredConstellation === c.name && (
            <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-70 text-white text-xs px-2 py-1 rounded pointer-events-none whitespace-nowrap">
              {c.name}
            </div>
          )}
        </div>
