    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto py-16 px-4">
          {/* Adjusted vertical margins for timeline line to account for image */}
          <div className="absolute left-1/2 transform -translate-x-1/2 top-0 bottom-1/4 h-full w-1 bg-gray-700 rounded-full"></div> {/* Central Timeline Line */}

          {educationItems.map((item, index) => (
            <div
              key={item.id}
              // Reduced mb (margin-bottom) for cards to make them closer
              className={`relative mb-6 flex items-center w-full ${
                index % 2 === 0 ? "justify-start" : "justify-end"
              }`}
            >
              <div
                // Adjusted vertical padding (py) for shorter cards
                // Increased horizontal padding (px) and width (w-6/12) for wider cards if content overflows
                className={`w-full md:w-6/12 p-4 rounded-lg shadow-lg border border-gray-700/70
              ...
              >
                <h3 className="text-xl md:text-2xl font-bold text-orange-400 mb-1"> {/* Reduced mb for tighter spacing */}
                  {item.degree}
                </h3>
                <p className="text-gray-200 text-lg mb-0.5">{item.institution}</p> {/* Reduced mb */}
                <p className="text-gray-400 text-sm italic mb-2">{item.year}</p> {/* Reduced mb */}
                {item.notes && (
        ...
        {/* Replaced "End of Transmissions" with educationEnd.png */}
        <div className="mt-8 relative z-10">
          <img
            src="/assets/educationEnd.png"
            alt="End of Education Timeline"
            className="max-w-xs md:max-w-sm h-auto mx-auto opacity-80" // Adjust size and opacity as needed
          />
        </div>
      </section>
    </>
