...
const ProjectCard = ({ project, activeIndex, index, totalProjects }) => {
  ...
  const getTransform = () => {
    ...
    if (absDistance === 1) {
      ...
      // This logic ensures that on small screens, translation is capped.
      // We'll use a `media query` for the `max-width` to apply this only on mobile.
...
const ProjectShowcase = () => {
  ...
  const handleTouchMove = (e) => {
    // Optionally, prevent default vertical scrolling if you only want horizontal swiping
    // const currentTouchX = e.targetTouches[0].clientX;
    // const diffX = currentTouchX - touchStartX;
    // if (Math.abs(diffX) > 10) { // Small threshold to allow some vertical scroll
    //   e.preventDefault();
    // }
  };
  ...
  return (
    <>
      <style jsx>{`
        ...
        /* Applying responsive translation directly in CSS for ProjectCard's style prop */
        @media (max-width: 767px) { /* Adjust based on your 'md' breakpoint, typically 768px */
            .ProjectCard > div { /* Target the inner div of ProjectCard for responsiveness */
                --mobile-cap-prefix: 0; /* Fallback, this will be overridden by direct styles */
            }
            .ProjectCard[data-distance="1"] {
                transform: translateX(min(120px, 80px)) scale(0.85) rotateY(-15deg) !important;
            }
            .ProjectCard[data-distance="-1"] {
                transform: translateX(max(-120px, -80px)) scale(0.85) rotateY(15deg) !important;
            }
            .ProjectCard[data-distance="2"] {
                transform: translateX(min(240px, 160px)) scale(0.7) rotateY(-25deg) !important;
            }
            .ProjectCard[data-distance="-2"] {
                transform: translateX(max(-240px, -160px)) scale(0.7) rotateY(25deg) !important;
            }
            /* You might need more specific rules for cards further out based on your exact layout */
        }
      `}</style>
