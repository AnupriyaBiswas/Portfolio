      ...
      <section className="relative pt-20 md:pt-32 min-h-screen overflow-hidden bg-black">
        ...
        <div className="relative z-10 px-4 md:px-16">
          ...
          <div className="flex justify-center mt-8 gap-6 flex-wrap md:hidden relative">
            ...
            {isDropdownOpen && (
              <div className="absolute top-full mt-2 w-full max-w-[200px] bg-gray-900 rounded-md shadow-lg z-20 border border-gray-700 animate-fadeInUp mx-auto overflow-hidden"> {/* Added overflow-hidden for rounded corners */}
                {categories.map((cat) => (
                  <button
                    key={cat}
                    className={`relative block w-full text-left px-4 py-3 text-base transition-colors duration-200
                      ...
                      ${selectedCategory === cat ? "before:scale-x-100" : "before:scale-x-0"}
                      `}
          ...
          <div className="hidden md:flex justify-center mt-8 gap-10 flex-wrap"> {/* Increased gap for more space */}
            {categories.map((cat) => (
              <button
                key={cat}
              ...
              >
                ...
                <span
                  className={`absolute bottom-0 left-0 w-full h-0.5 bg-orange-500 transition-transform duration-300 origin-center
                    ${selectedCategory === cat ? "scale-x-100" : "scale-x-0 group-hover:scale-x-50"}`} {/* Subtle hover effect */}
                ></span>
              </button>
            ))}
          </div>
