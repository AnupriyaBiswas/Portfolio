  ...
  return (
    <>
      <style jsx>{`
        ...
        @media (max-width: 767px) { /* Targeting screens up to 767px (before Tailwind's md breakpoint) */
            [data-distance="1"] {
                transform: translateX(-50%) translateX(80px) scale(0.85) rotateY(-15deg) !important;
            }
            [data-distance="-1"] {
                transform: translateX(-50%) translateX(-80px) scale(0.85) rotateY(15deg) !important;
            }
            [data-distance="2"] {
                transform: translateX(-50%) translateX(160px) scale(0.7) rotateY(-25deg) !important;
            }
            [data-distance="-2"] {
                transform: translateX(-50%) translateX(-160px) scale(0.7) rotateY(25deg) !important;
            }
            /* You may need to add rules for higher absolute distances if you have many projects
               and want to ensure they stay on screen or are hidden appropriately.
               Example for further cards (absDistance > 2):
            [data-distance="3"] { transform: translateX(-50%) translateX(210px) scale(0.55) rotateY(-35deg) !important; }
            [data-distance="-3"] { transform: translateX(-50%) translateX(-210px) scale(0.55) rotateY(35deg) !important; }
            */
        }
      `}</style>
        ...
        <div className="relative z-10 px-4 md:px-16">
          ...
          <div className="flex justify-center mt-8 gap-6 flex-wrap md:hidden relative">
            <button
              ...
              className="rounded-full px-6 py-2 text-sm transition-all duration-300 shadow-md font-semibold border border-orange-500 bg-orange-600 text-white hover:bg-orange-700 flex items-center justify-center gap-2 w-48 mx-auto" // Styled button
            >
              {selectedCategory === "All" ? "Categories" : selectedCategory}{" "} {/* Display selected category or "Categories" */}
              <span className="ml-2">
                <svg
            ...
            {isDropdownOpen && (
              <div className="absolute top-full mt-2 w-full max-w-[200px] bg-gray-900 rounded-md shadow-lg z-20 border border-gray-700 animate-fadeInUp mx-auto"> {/* Dropdown menu styling */}
                {categories.map((cat) => (
                  <button
                    key={cat}
                    className={`block w-full text-left px-4 py-3 text-base transition-colors duration-200 ${
                      selectedCategory === cat
                        ? "bg-orange-700 text-white font-bold"
                        : "text-gray-300 hover:bg-gray-700"
                    } first:rounded-t-md last:rounded-b-md`} // Rounded corners for first/last item
          ...
          <div className="hidden md:flex justify-center mt-8 gap-6 flex-wrap">
            {categories.map((cat) => (
              <button
                key={cat}
