            ...
            {researchItems.map((item, index) => (
              <div
                ...
                className={`group relative my-2 p-4 md:p-5 rounded-lg overflow-hidden
                                     bg-gradient-to-r from-blue-950/70 via-blue-950/60 to-transparent /* Changed to a very dark blue for background */
                                     border border-blue-800/70 /* Brighter, bluish default border */
                                     transition-all duration-500 cursor-pointer
                                     shadow-lg hover:shadow-xl hover:shadow-orange-500/40 /* Stronger hover shadow opacity */

                                     // Default (mobile) state: always visible, no hover effects
                                     opacity-100 scale-100
                                     
                                     // md and larger: apply initial subdued state and hover effects
                                     md:opacity-80 md:scale-95 /* Increased base opacity from 60 to 80 */
                                     md:hover:border-orange-600/80 /* Stronger hover border */
                                     md:hover:-translate-y-0.5
                                     md:hover:scale-100
                                     md:hover:shadow-orange-500/25 /* Slight adjustment here for consistency */
              ...
              >
                ...
                <div className="absolute inset-y-0 left-0 w-1/4 bg-orange-500/15 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300"> {/* Slightly higher opacity for stream */}
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-orange-500/40 to-transparent animate-[highlightStream_1.5s_ease-out_forwards_infinite_alternate]"></div> {/* Stronger stream color */}
                </div>
                ...
                <div className="absolute inset-0 border-2 border-transparent rounded-lg pointer-events-none
                                     md:group-hover:border-orange-500/80 transition-all duration-500" /* Even brighter hover border */
                  style={currentSpotlightIndex === index ? { animation: `subtleGlowPulse 2s infinite ease-in-out` } : {}}>
                </div>

                {/* Data Index / Chrono-Log Number */}
                <div className="absolute top-4 left-4 text-orange-200 text-xl font-bold opacity-80"> {/* Brighter orange, higher opacity for number */}
                  {index + 1}
                </div>

                {/* Content */}
                <div className="relative z-10 pl-12 md:pl-16">
                  <h3 className="text-lg md:text-xl font-bold mb-1 text-white group-hover:text-orange-50 transition-colors duration-300 leading-tight"> {/* Pure white, slightly brighter hover */}
                    {item.title}
                  </h3>

                  <p className="text-gray-100 mb-2 text-sm leading-relaxed line-clamp-2"> {/* Changed to gray-100 for very bright text */}
                    {item.summary}
                  </p>

                  <div className="flex items-center justify-between pt-2 border-t border-gray-600/70"> {/* Brighter, more visible border line */}
                    <a
                      href={item.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center px-4 py-1.5 bg-orange-500 text-white font-semibold rounded-full text-xs
                                                 hover:bg-orange-600 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-orange-500/40" /* Stronger button hover shadow */
                    >
                      View Research
                      <svg
                    ...
                    <div className="flex items-center space-x-1 text-xs">
                      <div
                        className={`w-2 h-2 rounded-full ${item.status === "Published"
                            ? "bg-green-300" /* Even brighter green */
                            : "bg-orange-300" /* Even brighter orange */
                          } animate-pulse-fast`}
                      ></div>
                      <span
                        className={`${item.status === "Published"
                            ? "text-green-200" /* Even brighter green text */
                            : "text-orange-200" /* Even brighter orange text */
                          } font-medium`}
                      >
                        {item.status}
                      </span>
