...
const Projects = () => {
  ...
  const getCardPosition = (index, totalCards) => {
    ...
    if (relativeIndex === 0) {
      ...
      x = position * 120 + (position * position * 20); // Curved spacing
      y = position * 15; // Slight vertical offset
      ...
      translateZ = -position * 150; // Deep stacking
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center justify-center min-h-[500px] mb-16" style={{perspective: '1000px'}}>
        {/* Navigation Arrows */}
        {filteredProjects.length > 1 && (
          <>
            <button
              ...
              className="absolute left-0 md:left-8 top-1/2 transform -translate-y-1/2 z-30 w-12 h-12 bg-gray-900/80 hover:bg-gray-800/80 text-white rounded-full border border-gray-700 hover:border-orange-500/50 transition-all duration-300 hover:scale-110 backdrop-blur-sm shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              onClick={nextCard}
              disabled={isAnimating}
              className="absolute right-0 md:right-8 top-1/2 transform -translate-y-1/2 z-30 w-12 h-12 bg-gray-900/80 hover:bg-gray-800/80 text-white rounded-full border border-gray-700 hover:border-orange-500/50 transition-all duration-300 hover:scale-110 backdrop-blur-sm shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <svg className="w-6 h-6 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="relative w-full h-full flex items-center justify-center preserve-3d">
          {filteredProjects.map((project, index) => {
            ...
            return (
              <div
                key={index}
                className={`absolute transition-all duration-800 ease-out cursor-pointer transform-gpu preserve-3d ${
                  isCenter ? 'pointer-events-auto' : 'pointer-events-auto'
                }`}
                style={{
                  transform: `translate3d(${position.x}px, ${position.y}px, ${position.translateZ}px) scale(${position.scale}) rotateY(${position.rotateY}deg)`,
              ...
              >
                <div
                  className={`w-[320px] min-h-[400px] bg-gradient-to-br from-gray-900/95 to-gray-800/95 backdrop-blur-sm rounded-3xl border overflow-hidden transition-all duration-500 transform-gpu ${
                    isCenter 
                      ? 'shadow-2xl shadow-orange-500/30 border-orange-500/40 hover:shadow-orange-500/40' 
                      : 'border-gray-700/50 hover:shadow-xl hover:shadow-orange-500/20 hover:border-orange-500/30'
                  }`}
                >
