...
const ProjectCard = ({ project, activeIndex, index, totalProjects }) => {
  ...
  const getTransform = () => {
    // Adjusted translateX values for better mobile fit and overall broader spread
    if (absDistance === 0) return "translateX(0) scale(1) rotateY(0deg)";
    if (absDistance === 1) {
      return distance > 0
        ? "translateX(100px) md:translateX(180px) scale(0.85) rotateY(-15deg)" // Reduced for mobile
        : "translateX(-100px) md:translateX(-180px) scale(0.85) rotateY(15deg)"; // Reduced for mobile
    }
    if (absDistance === 2) {
      return distance > 0
        ? "translateX(200px) md:translateX(360px) scale(0.7) rotateY(-25deg)" // Reduced for mobile
        : "translateX(-200px) md:translateX(-360px) scale(0.7) rotateY(25deg)"; // Reduced for mobile
    }
    // For cards further out, ensure they are still visible but scaled down
    return distance > 0
      ? `translateX(${200 + (absDistance - 2) * 60}px) md:translateX(${360 + (absDistance - 2) * 120}px) scale(0.55) rotateY(-35deg)`
      : `translateX(${-200 - (absDistance - 2) * 60}px) md:translateX(${-360 - (absDistance - 2) * 120}px) scale(0.55) rotateY(35deg)`;
  };
    ...
    <>
      ...
      <section className="relative pt-20 md:pt-32 min-h-screen overflow-hidden bg-black">
        ...
        <div className="relative z-10 px-4 md:px-16">
          ...
          <div className="flex justify-start md:justify-center mt-8 gap-4 md:gap-6 px-4 md:px-0 overflow-x-auto pb-4 scrollbar-hide"> {/* Added overflow-x-auto, pb-4, scrollbar-hide, adjusted gap and justify */}
            {categories.map((cat) => (
              <button
                key={cat}
                className={`flex-shrink-0 rounded-full px-6 py-2 md:px-8 md:py-3 text-sm md:text-lg transition-all duration-500 shadow-lg font-semibold border-2 transform hover:scale-105 ${ // Adjusted padding and text size
        ...
        <div className="relative w-full mt-10 md:mt-20 px-4"> {/* Adjusted top margin for carousel */}
          <div
            className="relative h-[400px] md:h-[500px] max-w-full md:max-w-7xl mx-auto" {/* Adjusted height and max-width for mobile */}
          ...
          >
            ...
            <button
              ...
              className="absolute left-2 top-1/2 transform -translate-y-1/2 z-50 bg-gradient-to-r from-gray-800/90 to-gray-900/90 backdrop-blur-sm p-2 md:p-4 rounded-full shadow-2xl border border-gray-600 hover:border-orange-500 transition-all duration-300 hover:scale-110" {/* Adjusted padding for mobile */}
            >
              <ChevronLeft className="text-white w-5 h-5 md:w-6 md:h-6" /> {/* Adjusted icon size */}
            </button>

            <button
              onClick={handleNext}
              className="absolute right-2 top-1/2 transform -translate-y-1/2 z-50 bg-gradient-to-r from-gray-800/90 to-gray-900/90 backdrop-blur-sm p-2 md:p-4 rounded-full shadow-2xl border border-gray-600 hover:border-orange-500 transition-all duration-300 hover:scale-110" {/* Adjusted padding for mobile */}
            >
              <ChevronRight className="text-white w-5 h-5 md:w-6 md:h-6" /> {/* Adjusted icon size */}
            </button>
          </div>

          {/* Pagination Dots */}
          <div className="flex justify-center mt-8 md:mt-12 gap-3"> {/* Adjusted top margin for dots */}
