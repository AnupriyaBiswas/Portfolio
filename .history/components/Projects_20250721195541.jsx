...
const ProjectCard = ({ project, activeIndex, index, totalProjects }) => {
  ...
  const getTransform = () => {
    // Reverted to original broad spread values for desktop, and added mobile specific reductions
    if (absDistance === 0) return "translateX(0) scale(1) rotateY(0deg)";
    if (absDistance === 1) {
      return distance > 0
        ? "translateX(60px) md:translateX(100px) scale(0.85) rotateY(-15deg)" // Smaller translate for mobile
        : "translateX(-60px) md:translateX(-100px) scale(0.85) rotateY(15deg)"; // Smaller translate for mobile
    }
    if (absDistance === 2) {
      return distance > 0
        ? "translateX(120px) md:translateX(200px) scale(0.7) rotateY(-25deg)" // Smaller translate for mobile
        : "translateX(-120px) md:translateX(-200px) scale(0.7) rotateY(25deg)"; // Smaller translate for mobile
    }
    // For cards further out, maintain a reasonable spread but still reduce on mobile
    return distance > 0
      ? `translateX(180px) md:translateX(${200 + (absDistance - 2) * 80}px) scale(0.55) rotateY(-35deg)` // Adjusted for mobile and further spread on desktop
      : `translateX(-180px) md:translateX(${-200 - (absDistance - 2) * 80}px) scale(0.55) rotateY(35deg)`; // Adjusted for mobile and further spread on desktop
  };
