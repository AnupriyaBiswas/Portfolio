    ...
    const generateStars = () => {
      ...
      for (let i = 0; i < 150; i++) {
        newStars.push({
          ...
          fallDuration: 25 + Math.random() * 20, // New: separate duration for fall
          twinkleDuration: 6 + Math.random() * 8, // New: separate duration for twinkle
        });
      }
      setStars(newStars);
    };
            ...
            {researchItems.map((item, index) => (
              ...
              >
                ...
                <div className="absolute top-0 bottom-0 left-0 w-16 md:w-20 flex items-center text-orange-200 text-6xl md:text-7xl font-bold opacity-50 pointer-events-none">
                    <span className="leading-none pl-4">{index < 9 ? `0${index + 1}` : index + 1}</span>
                </div>
