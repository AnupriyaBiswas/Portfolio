...
const ProjectCard = ({ project, activeIndex, index, totalProjects, onClick }) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-4 w-[220px] sm:w-[260px] md:w-[320px] text-white shadow-2xl border border-gray-700 hover:border-orange-500 transition-all duration-500">
        ...
        <h3 className="text-base sm:text-lg font-bold mb-2 text-white">{project.title}</h3>
        ...
        <a
          ...
          className="inline-flex items-center bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 transition-all duration-300 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-white font-medium text-xs sm:text-sm shadow-lg hover:shadow-orange-500/30"
        >
          Visit Project →
        </a>
      </div>
    ...
    <>
      ...
      <section className="relative pt-20 md:pt-32 min-h-screen overflow-hidden transparent">
        ...
        <div className="relative z-10 px-4 md:px-16 mt-0">
          <h1 className="text-3xl sm:text-4xl md:text-8xl font-bold leading-tight text-center mb-6">
            <span className="text-orange-500">PROJECT </span>
            <span className="text-white">SHOWCASE</span>
          </h1>

          {/* Categories */}
          <div className="flex justify-center mt-4 gap-4 sm:gap-6 md:gap-10 flex-wrap z-10">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setSelectedCategory(cat)}
                className={`relative px-2 sm:px-3 md:px-4 py-1 sm:py-2 text-sm sm:text-base md:text-xl font-semibold transition-all duration-300 group ${selectedCategory === cat
                  ? "text-orange-500"
        ...
        <div className="relative w-full mt-8 md:mt-6 px-2 sm:px-4 z-10">
          <div
            className="relative h-[340px] sm:h-[400px] md:h-[460px] max-w-full md:max-w-7xl mx-auto flex items-center justify-between"
          ...
          >
            ...
            <div className="relative flex justify-center items-center h-full w-full mt-4 sm:mt-6 md:mt-0">
          ...
          {/* Pagination */}
          <div className="flex justify-center mt-6 md:mt-10 gap-2 md:gap-3 z-10">
            {filteredProjects.map((_, index) => (
              <button
                key={index}
                onClick={() => setActiveIndex(index)}
                className={`transition-all duration-500 rounded-full ${index === activeIndex
                  ? "w-4 h-2 sm:w-5 sm:h-2 md:w-8 md:h-3 bg-orange-500"
                  : "w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3 bg-gray-600 hover:bg-gray-400"
                  }`}
              />
            ))}
          </div>
